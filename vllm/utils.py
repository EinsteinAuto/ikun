...
def bind_kv_cache_scale(
        ctx: dict[str, Any],
        kv_cache_scale: list[list[torch.Tensor]],  # [virtual_engine][layer_index]
) -> None:
    # Bind the kv_cache tensor to Attention modules, similar to
    # ctx[layer_name].kv_cache[ve]=kv_cache[ve][extract_layer_index(layer_name)]
    # Special things handled here:
    ...
    layer_index_sorted = sorted(
        set(
            extract_layer_index(layer_name)
            for layer_name in layer_need_kv_cache_scale))
    for layer_name in layer_need_kv_cache_scale:
        kv_cache_idx = layer_index_sorted.index(
            extract_layer_index(layer_name))
        forward_ctx = ctx[layer_name]
        assert len(forward_ctx.kv_cache_scale) == len(kv_cache_scale)
        for ve, ve_kv_cache_scale in enumerate(kv_cache_scale):
            forward_ctx.kv_cache_scale[ve] = ve_kv_cache_scale[kv_cache_idx]
